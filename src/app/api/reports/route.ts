...
import { db } from '@/db';
import { receivings, receivingSessions, sensorEvents, lines, trucks, suppliers } from '@/db/schema';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const formattedList = await Promise.all(completedReceivings.map(async (rec) => {
      totalManifest += rec.manifestCount;
      let actualCount = 0;
      
      const session = await db.query.receivingSessions.findFirst({
        where: eq(receivingSessions.receivingId, rec.id)
      });
      ...
      const differencePercent = rec.manifestCount > 0 
    ...
    const totalDifferencePercent = totalManifest > 0 
      ? Number(((totalDifference / totalManifest) * 100).toFixed(2)) 
      : 0;

    // Assigned vs Unassigned Detections
    const detectionsConditions = [
      eq(sensorEvents.eventType, 'DETECTION'),
      eq(sensorEvents.eventMode, 'PRODUCTION')
    ];
    if (dateFrom) detectionsConditions.push(gte(sensorEvents.deviceTime, new Date(dateFrom)));
    if (dateTo) detectionsConditions.push(lte(sensorEvents.deviceTime, new Date(dateTo)));
